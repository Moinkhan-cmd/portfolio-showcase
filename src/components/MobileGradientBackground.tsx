...
// Ultra-lightweight mobile background - NO framer-motion
export const MobileGradientBackground = memo(({ 
  ...
  return (
    <div 
      ...
      style={{ zIndex: 0 }}
    >
      {/* Primary gradient - top corner - static, no animation */}
      <div
        className={`absolute -top-1/4 -right-1/4 w-[80%] h-[80%] bg-gradient-to-bl ${gradients.primary} opacity-70`}
        style={{ filter: 'blur(40px)' }}
      />
      
      {/* Secondary gradient - bottom corner - static */}
      <div
        className={`absolute -bottom-1/4 -left-1/4 w-[70%] h-[70%] bg-gradient-to-tr ${gradients.secondary} opacity-60`}
        style={{ filter: 'blur(40px)' }}
      />
      
      {/* Center accent glow - static */}
      <div
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[60%] h-[60%] rounded-full opacity-50"
        style={{ 
